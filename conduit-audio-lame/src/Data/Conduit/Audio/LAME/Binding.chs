...
-- | as 'encodeBuffer', but for 'CFloat's.
...
-- | as 'encodeBuffer', but for 'CDouble's.
-- !! NOTE: !! data must be scaled to +/- 1 full scale
...
{- | as 'encodeBuffer', but for 'CLong's.
!! NOTE: !! data must still be scaled to be in the same range as
'CShort', @+/- 32768@

This scaling was a mistake (doesn't allow one to exploit full
precision of type 'CLong'.  Use 'encodeBufferLong2' instead.
-}
...
{- |
Same as 'encodeBufferLong', but with correct scaling.
!! NOTE: !! data must still be scaled to be in the same range as
type 'CLong'.   Data should be in the range:  @+/- 2^(8*size(long)-1)@
-}
...
{- |
as 'encodeBuffer', but for 'CInt's.
!! NOTE: !! input should be scaled to the maximum range of 'CInt'.
If 'CInt' is 4 bytes, then the values should range from
+/- 2147483648.

This routine does not (and cannot, without losing precision) use
the same scaling as the rest of the 'encodeBuffer' routines.
-}
