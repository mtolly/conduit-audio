...
formatToInt :: (Integral a) => Format -> a
formatToInt fmt = case fmt of
  AO_FMT_LITTLE -> {#const AO_FMT_LITTLE #}
  AO_FMT_BIG    -> {#const AO_FMT_BIG    #}
  AO_FMT_NATIVE -> {#const AO_FMT_NATIVE #}
...
withSampleFormat fmt act = do
  ...
  {#set ao_sample_format.byte_format #} p $ formatToInt $ byteFormat fmt
  (case matrix fmt of
    Nothing -> ($ nullPtr)
